#!/usr/bin/env python3
"""terrain_voxel_v5.pyの490-535行目を修正版に置き換え"""

# 修正後のコード
new_code = '''                # === ステップ3: 隣接区分との補間（区分境界を滑らかに） ===
...
# ファイルを読み込み
with open('terrain_voxel_v5.py', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# 490行目から535行目（0-indexed: 489-534）を置き換え
# 新しいコードを行リストに変換
...
# 置き換え実行
result_lines = lines[:489] + new_lines + lines[535:]

# ファイルに書き込み
with open('terrain_voxel_v6.py', 'w', encoding='utf-8') as f:
    f.writelines(result_lines)

print(f"✅ 修正完了: terrain_voxel_v6.py")
print(f"   置き換え: 490-535行目 ({535-489}行) → {len(new_lines)}行")
